...
| `DASHBOARD_DISCOVERY_PORT` | UDP 发现端口，默认 `39300`；设为 `0` 可关闭发现 |
...
6. **UDP 发现暴露面**：启用 UDP 发现时，局域网内任何主机都可探测服务是否存在并获取候选 IPv4 列表与推荐显示 URL（不含敏感数据，但会暴露服务存在性）。
   - 若不需要自动发现：建议设置 `DASHBOARD_DISCOVERY_PORT=0` 关闭，减少暴露面与日志噪声。
   - 仍需启用：建议仅在可信局域网/VLAN 使用，并在主机防火墙限制 UDP 39300 入站来源网段。
